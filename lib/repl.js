...
var print_expr = function (obj) {
  if (obj && _.isEqual(obj.type, 'id')) {
    process.stdout.write("\033[36m'" + obj.name + "\033[39m\n");
  }
  else {
    process.stdout.write(util.inspect(obj, false, 0, true) + "\n");
  }
}
  ...
  io.on('line', function (input) {
    if (buffer !== "") {
      ...
      if (buf_missing_rp + missing_rp(input) <= 0) {
        print_expr(e.evalAll(r.parse(buffer)));
        buffer = "";
      ...
      if (missing_rp(input) <= 0)
        print_expr(e.evalAll(r.parse(input)));
      else {
